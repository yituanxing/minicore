...
int main(int argc, char** argv) {
  try {
    ...
    const std::uint64_t maxCycles =
        argc == 3 ? std::stoull(argv[2], nullptr, 0) : 12000000ULL;
    ...
    for (; cycles < maxCycles; ++cycles) {
      ...
      if (top.io_commit_valid) {
        ...
        if (pc == kLinuxPhysicalEntry) {
          std::cerr << "\nL32_LINUX_ENTRY_PASS cycles=" << cycles
                    << " commits=" << commits
    ...
    std::cerr << "\nL32_LINUX_HANDOFF_TIMEOUT cycles=" << cycles
              << " commits=" << commits
    ...
    std::cerr << "L32_LINUX_HANDOFF_RUNNER_ERROR: " << error.what() << "\n";
    return 1;
  }
}
