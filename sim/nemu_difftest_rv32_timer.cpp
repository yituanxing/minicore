...
constexpr std::size_t kTraceDepth = 40;
constexpr std::size_t kMmioPageSize = 4096;
...
char kPlatformMmioName[] = "aethercore-rv32-platform-mmio";
char kMtimecmpMmioName[] = "aethercore-rv32-mtimecmp-mmio";
char kMtimeMmioName[] = "aethercore-rv32-mtime-mmio";
...
Function loadSymbol(void* handle, const char* name) {
  ...
  if (const char* error = dlerror()) {
    throw std::runtime_error(std::string("RV32 timer DiffTest missing NEMU symbol ") +
                             name + ": " + error);
  }
...
class NemuDifftest::Impl {
 public:
  ...
  Impl(const std::string& sharedObject, const std::string& imagePath,
    ...
    try {
      ...
      init_();
      platformMmio_ = addPassiveMap(kPlatformMmioName, kPlatformMmioBase);
      mtimecmpMmio_ = addPassiveMap(kMtimecmpMmioName, kMtimecmpPageBase);
      mtimeMmio_ = addPassiveMap(kMtimeMmioName, kMtimePageBase);
 ...
 private:
  ...
  std::uint8_t* addPassiveMap(char* name, std::uint32_t base) {
    auto* space = newSpace_(static_cast<int>(kMmioPageSize));
    if (space == nullptr) {
      throw std::runtime_error("RV32 timer DiffTest failed to allocate reference MMIO space");
    }
    std::memset(space, 0, kMmioPageSize);
    addMmioMap_(name, base, space, static_cast<int>(kMmioPageSize), nullptr);
    return space;
  }
  ...
  std::uint8_t* mappedPointer(std::uint32_t address, std::size_t size) const {
    const auto inPage = [&](std::uint32_t base) {
      return address >= base &&
             static_cast<std::uint64_t>(address - base) + size <= kMmioPageSize;
    };
    if (inPage(kPlatformMmioBase)) return platformMmio_ + (address - kPlatformMmioBase);
    if (inPage(kMtimecmpPageBase)) return mtimecmpMmio_ + (address - kMtimecmpPageBase);
    if (inPage(kMtimePageBase)) return mtimeMmio_ + (address - kMtimePageBase);
    return nullptr;
  }
  ...
  NemuState32 executeZicsr(const NemuState32& before, std::uint32_t instruction,
    ...
    if (operation == 0) fail("timer Zicsr shadow received reserved SYSTEM funct3");
    ...
    if (writeIntent && !csrWritable(address)) {
      fail("timer Zicsr shadow received a normal write to read-only CSR " + hex32(address));
    }
  ...
  NemuState32 executeMret(const NemuState32& before, const DifftestCommit& commit) {
    if (commit.inst != kMret || commit.rdWrite || commit.memValid || commit.interrupt) {
      fail("timer MRET shadow received an invalid architectural event");
    }
    machine_.mstatus =
  ...
  void validateInterrupt(const DifftestCommit& commit) const {
  ...
  void observeTimerStore(const DifftestCommit& commit) {
    ...
    if (address >= kMtimecmpAddress && address < kMtimecmpAddress + 8U) {
      // Workloads use the standard RV32 safe update sequence. Any compare write
      // moves the comparator away from the already-fired value before the next
      // mip read or MRET boundary.
  ...
  [[noreturn]] void fail(const std::string& reason) const {
    std::ostringstream out;
    out << "RV32 timer DiffTest mismatch after " << checked_
        << " matched events: " << reason;
    if (!trace_.empty()) {
  ...
  std::uint8_t* mtimecmpMmio_ = nullptr;
  std::uint8_t* mtimeMmio_ = nullptr;
