...
// Re-drive host memory after the first low-phase eval and request a second
// eval only when the DUT changed a request enough to change host feedback.
// This keeps the qualified two-eval behavior whenever external information
// actually changes while skipping the redundant settle pass on stable cycles.
template <typename Top>
