...
def main() -> None:
    ...
    ordered_names = required[1:]
    ordered = [labels[name] for name in ordered_names]
    if ordered != sorted(ordered):
        raise SystemExit("kernel/user labels are not in the expected order")
    if any(address % 4 for address in (labels[name] for name in required)):
    ...
    # The bootstrap retires every word from reset through its MRET. The first
    # user path is straight-line through the first ECALL, so this formula pins
    # the precise DiffTest event at which the privilege transition is tested.
    bootstrap_retirements = (labels["bootstrap_returned"] - BASE) // 4
    first_user_ecall = ecall_sites[0]
    first_user_ecall_event = bootstrap_retirements + (first_user_ecall - user_start) // 4
    ...
    output_path.write_text(
        "\n".join(
            (
                ...
                f"first_user_ecall=0x{first_user_ecall:08x}",
                f"first_user_ecall_event={first_user_ecall_event}",
