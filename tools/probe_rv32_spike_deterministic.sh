...
WORK_DIR="${1:-${AETHERCORE_RV32_SPIKE_WORK_DIR:-$ROOT/build/rv32-spike-reference}}"
...
for tool in git make g++ strings file nm sha256sum awk; do
...
# invocation proves this assumption wrong and fails the build.
...
g++ --version > "$EVIDENCE_DIR/host-cxx-version.txt"
