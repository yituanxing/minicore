...
LINUX_BUILD_DIR="${ROOT_DIR}/build/rv64-busybox-shell"
LINUX_IMAGE="${LINUX_BUILD_DIR}/obj/arch/riscv/boot/Image"
BUILD_DIR="${ROOT_DIR}/build/rv64-busybox-shell-boot"
OPENSBI_BUILD_DIR="${BUILD_DIR}/opensbi"
EVIDENCE_DIR="${BUILD_DIR}/evidence"
DTS="${BUILD_DIR}/aethersoc-v0-busybox.dts"
DTB="${BUILD_DIR}/aethercore-rv64-busybox.dtb"
JOBS="${RV64_LINUX_JOBS:-$(nproc)}"
BOOTARGS="${RV64_LINUX_BOOTARGS} rdinit=/init"

fail() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }

grep -qx 'RV64_BUSYBOX_SHELL_BUILD_RESULT: status=PASS' "${LINUX_BUILD_DIR}/result.txt" || fail "RV64 BusyBox shell Image is not qualified"
[[ -s "${LINUX_IMAGE}" ]] || fail "RV64 BusyBox shell Image missing"
actual_image_sha="$(sha256sum "${LINUX_IMAGE}" | awk '{print $1}')"
recorded_image_sha="$(sed -n 's/^image_sha256=//p' "${LINUX_BUILD_DIR}/result.txt" | head -n1)"
[[ -n "${recorded_image_sha}" && "${actual_image_sha}" == "${recorded_image_sha}" ]] || fail "RV64 BusyBox shell Image hash drifted"

[[ -d "${OPENSBI_SOURCE}/.git" ]] || fail "qualified OpenSBI source cache missing"
[[ "$(git -C "${OPENSBI_SOURCE}" rev-parse HEAD)" == "${RV64_OPENSBI_COMMIT}" ]] || fail "OpenSBI commit drifted"
git -C "${OPENSBI_SOURCE}" diff --quiet --ignore-submodules -- || fail "OpenSBI source cache dirty"

rm -rf "${BUILD_DIR}"
mkdir -p "${OPENSBI_BUILD_DIR}" "${EVIDENCE_DIR}"

command -v dtc >/dev/null 2>&1 || fail "device-tree-compiler required"
...
"${ROOT_DIR}/mill" aethercore.runMain aethercore.EmitAetherSoCDts \
  ...
  --bootargs "${BOOTARGS}" \
  | tee "${EVIDENCE_DIR}/aethersoc-rv64-busybox-dts.txt"
...
[[ -s "${FW_ELF}" && -s "${FW_BIN}" ]] || fail "missing RV64 OpenSBI+BusyBox outputs"
...
[[ "$((entry))" -eq "$((RV64_OPENSBI_FW_TEXT_START))" ]] || fail "OpenSBI entry drifted"
...
{
  echo "RV64_BUSYBOX_SHELL_PAYLOAD_BUILD_RESULT: status=PASS"
