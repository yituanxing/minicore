...
LINUX_BUILD_DIR="${ROOT_DIR}/build/rv64-linux-busybox"
...
grep -qx 'RV64_BUSYBOX_INITRAMFS_BUILD_RESULT: status=PASS' "${LINUX_BUILD_DIR}/result.txt" || \
  fail "RV64 BusyBox initramfs Image is not qualified"
[[ -s "${LINUX_IMAGE}" ]] || fail "RV64 BusyBox initramfs Image is missing"
...
[[ -n "${recorded_image_sha}" && "${actual_image_sha}" == "${recorded_image_sha}" ]] || \
  fail "RV64 BusyBox initramfs Image hash drifted"
...
python3 "${ROOT_DIR}/tools/ci/make_l32_dtb.py" \
  --output "${DTB}" \
  --summary "${EVIDENCE_DIR}/aethercore-rv64-busybox-dtb.txt" \
...
{
  echo "RV64_BUSYBOX_PAYLOAD_BUILD_RESULT: status=PASS"
