...
for real in "${LUA_ELF}" "${SQLITE_ELF}" "${BASH_ELF}" "${BUSYBOX_REAL_ELF}" "${ZLIB_ELF}" "${LUA_SMOKE}" "${BASH_SMOKE}"; do
...
sha256sum \
  "${BUSYBOX_ELF}" "${PROBE_ELF}" "${LUA_ELF}" "${SQLITE_ELF}" "${BASH_ELF}" "${BUSYBOX_REAL_ELF}" "${ZLIB_ELF}" \
