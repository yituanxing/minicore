...
grep -qx 'L32_BUSYBOX_BUILD_RESULT: status=PASS' "${BUSYBOX_BUILD_DIR}/result.txt" || {
  echo "ERROR: qualified L32 BusyBox build is required" >&2
  exit 22
}
...
[[ -n "${recorded_busybox_sha}" && "${actual_busybox_sha}" == "${recorded_busybox_sha}" ]] || {
  echo "ERROR: qualified BusyBox ELF hash drifted" >&2
  exit 23
}
...
grep -Fqx "CONFIG_INITRAMFS_SOURCE=\"${INIT_SPEC}\"" "${OBJ_DIR}/.config" || {
  echo "ERROR: Linux config did not retain the BusyBox initramfs source" >&2
  exit 24
}
...
[[ -s "${VMLINUX}" && -s "${IMAGE}" ]] || {
  echo "ERROR: BusyBox initramfs Linux build did not produce vmlinux/Image" >&2
  exit 25
}
