...
BUILD_DIR="${ROOT_DIR}/build/l32-linux-boot"
...
make -C "${SOURCE_DIR}" \
  ...
  -j"${JOBS}" 2>&1 | tee "${BUILD_DIR}/opensbi-linux-build.log"
...
[[ -s "${FW_ELF}" && -s "${FW_BIN}" ]] || {
  echo "ERROR: missing OpenSBI+Linux payload outputs" >&2
  exit 25
}
...
# RV32 Linux must enter at a 4 MiB PMD boundary. Keep the handoff explicit and
# fail closed instead of relying on the OpenSBI test-payload default.
[[ "$((FW_TEXT_START + FW_PAYLOAD_OFFSET))" -eq "$((0x80400000))" ]] || exit 27

{
  echo "L32_LINUX_PAYLOAD_BUILD_RESULT: status=PASS"
