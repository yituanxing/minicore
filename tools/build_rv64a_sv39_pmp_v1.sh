...
out_dir="${1:-build/rv64a-sv39-pmp-v1/software}"
...
elf="$out_dir/rv64a-sv39-pmp-v1.elf"
bin="$out_dir/rv64a-sv39-pmp-v1.bin"

"$cc" "${cflags[@]}" software/rv64/rv64a_sv39_pmp_v1_workload.S \
  "${ldflags[@]}" -Wl,-Map,"$out_dir/rv64a-sv39-pmp-v1.map" -o "$elf"

"$objcopy" -O binary "$elf" "$bin"
"$objdump" -d -S "$elf" > "$out_dir/rv64a-sv39-pmp-v1.dis"
"$readelf" -h -l -S -A "$elf" > "$out_dir/rv64a-sv39-pmp-v1.elf.txt"
"$nm" -n "$elf" > "$out_dir/rv64a-sv39-pmp-v1.nm"

grep -q 'Class:[[:space:]]*ELF64' "$out_dir/rv64a-sv39-pmp-v1.elf.txt" || {
  echo 'ERROR: RV64A Sv39 PMP V1 workload is not ELF64' >&2
  exit 1
}
for mnemonic in mret sret ecall ld sd; do
  grep -Eq "[[:space:]]${mnemonic}([[:space:]]|$)" "$out_dir/rv64a-sv39-pmp-v1.dis" || {
    echo "ERROR: RV64A Sv39 PMP V1 workload is missing required instruction: $mnemonic" >&2
    exit 1
  }
done
...
grep -Eq '[[:space:]]sfence\.vma([[:space:]]|$)' "$out_dir/rv64a-sv39-pmp-v1.dis" || {
  echo 'ERROR: RV64A Sv39 PMP V1 workload is missing sfence.vma' >&2
  exit 1
}
for csr in satp pmpcfg0 pmpaddr0 mtvec stvec medeleg mstatus mepc sstatus sepc scause stval sscratch; do
  grep -q "$csr" "$out_dir/rv64a-sv39-pmp-v1.dis" || {
    echo "ERROR: RV64A Sv39 PMP V1 workload is missing required CSR: $csr" >&2
    exit 1
  }
done
...
symbol_addr() {
  local name="$1"
  awk -v name="$name" '$3 == name { print "0x" $1; exit }' "$out_dir/rv64a-sv39-pmp-v1.nm"
}
...
[[ "$(symbol_addr user_atomic_word)" == 0x0000000080003008 ]] || {
  echo "ERROR: user atomic word drifted: $(symbol_addr user_atomic_word)" >&2
  exit 1
}
...
{
  echo 'contract=rv64a-sv39-pmp-v1-executable'
  echo 'march=rv64ima_zicsr'
  echo 'mabi=lp64'
  echo 'modes=M,S,U'
  echo 'vm=Sv39'
  echo 'page_levels=3'
