...
out_dir="${1:-build/rv32im-embench/software}"
...
benchmarks=(crc32 edn matmult-int statemate)
...
for benchmark in "${benchmarks[@]}"; do
  ...
  cp "$source_dir"/*.c "$scaled_dir/"

  # Embench local scale factors normalize timed runs to seconds. For this
  # correctness corpus, execute one algorithm iteration and retain the exact
  # mechanical patch. Inputs and verify_benchmark() remain untouched.
