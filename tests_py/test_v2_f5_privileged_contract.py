...
class V2F5PrivilegedContractTest(unittest.TestCase):
    ...
    def test_privileged_recovery_clears_speculative_dependency_state(self):
        ...
        self.assertIn("private val rename = Mem(32, new ProducerTag", text)
        self.assertIn("producer.valid", text)
        self.assertIn("producer.rd === address", text)
        self.assertIn("sameProducer(producer.producerTag, mapping)", text)
        self.assertNotIn("rename(register).valid := false.B", text)
