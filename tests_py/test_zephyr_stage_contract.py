...
class ZephyrStageContractTest(unittest.TestCase):
    ...
    def test_host_gate_builds_without_the_minicore_runner(self) -> None:
        ...
        self.assertIn("context: 'zephyr/host-gate'", text)
        self.assertIn("state: 'pending'", text)
        self.assertIn("Publish final Zephyr host status", text)
        self.assertIn("actions/runs/${process.env.GITHUB_RUN_ID}", text)
        self.assertIn("path: minicore", text)
