...
class AetherSoCInstructionCache(
  ...
  private val missOldData = Reg(UInt(dataBits.W))
  ...
  when(io.request.fire) {
    ...
    missMask := reqMask
    missSameLine := tagHit
    missOldData := Mux(tagHit, lineData(reqIndex), 0.U)
  ...
  when(io.response.fire) {
    ...
    when(missFillValid && !io.response.bits.fault && !io.invalidateAll) {
      ...
      lineData.write(
        missIndex,
        mergeBytes(Mux(missSameLine, missOldData, 0.U), shifted, missMask)
      )
      lineByteValid.write(
        missIndex,
        Mux(missSameLine, missOldMask, 0.U) | missMask
      )
    }
