...
/**
  * Fast incremental-cost synthesis boundary for the stage-1 RV64 D-cache.
  *
  * This intentionally measures only the hardware newly introduced by #212.
  * The pre-existing 2-entry platform pending queue already exists in frozen
  * #210 and is therefore not charged to the D-cache delta. Host-only hit/miss/
  * bypass counters are not exposed at this boundary and can be removed by
  * synthesis.
  */
