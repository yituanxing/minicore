...
class TinyDependencyState(val xlen: Int) extends Module {
  ...
  when(io.recovery.valid) {
    ...
    // Normal branch recovery keeps only the already-completed head lifetime.
    // Its source operands are no longer needed for execution, but the read-only
    // scheduling/head views remain live until retirement. Freeze the currently
    // materialized values at the recovery boundary before clearing producer
    // state so those views cannot lose a retired producer that has not yet been
    // reused into retiredProducerValues.
    val survivorDependency = dependencies(survivor.robToken.index)
    when(
      survivorDependency.valid &&
        sameRobToken(survivorDependency.robToken, survivor.robToken)
    ) {
      dependencies(survivor.robToken.index).rs1.value := io.headRs1.value
      dependencies(survivor.robToken.index).rs2.value := io.headRs2.value
      dependencies(survivor.robToken.index).rs1ValueSnapshotValid := true.B
      dependencies(survivor.robToken.index).rs2ValueSnapshotValid := true.B
    }
