...
class TinyMemoryBackend(
  ...
  // Branch and Memory still exclude all compute launch through selectiveIssue.block.
  // The only widened case is two side-effect-free compute launches: the primary
  // selective lane plus one distinct Integer-only shadow lane.
  assert(!(selectiveIssue.io.secondaryIntegerRequest.fire &&
    !selectiveIssue.io.request.fire),
    "secondary Integer launch requires a same-cycle primary compute launch")
  assert(PopCount(Cat(
    branchIssue.io.request.fire,
    selectiveIssue.io.request.fire,
    selectiveIssue.io.secondaryIntegerRequest.fire,
    lsu.io.request.fire
  )) <= 2.U, "A8 dual-compute experiment may launch at most two uOps per cycle")
