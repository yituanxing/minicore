...
/**
  * Architectural instruction-semantic boundary for v2.
  *
  * Encoding/profile legality and architectural meaning terminate here. The
  * output deliberately contains no ROB identity, execution class, queue/port
  * choice, scheduler state, producer tag or value-storage identity.
  *
  * The qualified shared Decoder remains the encoding owner while the v2
  * representation translates its pipeline-oriented control fields into the
  * stable DecodedInstruction contract consumed by the backend classifier.
  */
class TinyArchitecturalSemanticDecode(val isa: IsaConfig) extends Module {
  ...
  // SFENCE.VMA is an architectural SYSTEM encoding whose rs1/rs2 fields are
  // semantic operands. Privilege legality remains enforced by the qualified
  // privileged/commit path; recognizing the family here prevents any backend
  // stage from re-decoding the raw instruction.
  ...
  // Preserve SFENCE.VMA dependency facts even though the current VM path uses
  // conservative whole-translation invalidation. Selective VA/ASID invalidation
  // must not require backend opcode re-decode later.
