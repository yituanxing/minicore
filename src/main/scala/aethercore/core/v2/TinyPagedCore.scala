...
class TinyPagedCore(
  ...
  // lifetime, so one Supervisor-mode PMP checker can qualify the selected
  // request after arbitration. TinyPagedCore opts its backend into external
  // PTW-PMP ownership above; standalone backends retain their local checker.
