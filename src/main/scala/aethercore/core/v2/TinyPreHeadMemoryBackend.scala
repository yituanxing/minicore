...
class TinyPreHeadMemoryBackend(
  ...
  // feeding acceptedPrivilegedRecovery back into this request.valid would form
  // completion -> LSU request -> selector block -> completion combinationally.
  // The selector policy itself already makes Branch, System, older Memory,
  // non-Normal ordering and known exceptions hard barriers for younger Loads.
  // branchRequest.valid is retained as the explicit independent launch-owner
  // cut and preserves the global one-launch invariant without depending on a
  // completion generated by this LSU request.
