...
import aethercore.core.{Rv32CDecompressor, RvcDecompressor}

class Rv32CDecompressorSpec extends AnyFlatSpec with Matchers with ChiselSim {
  behavior of "XLEN-aware RVC decompression"
  ...
  it should "reject reserved, custom, non-integer and non-RVC encodings at RV32" in {
    val illegal = Seq(
  ...
  it should "expand representative RV64C-only integer encodings" in {
    simulate(new RvcDecompressor(64)) { dut =>
      for (entry <- rv64OnlyCases) {
        withClue(entry.name) {
          dut.io.raw.poke(entry.raw.U(16.W))
          dut.io.legal.expect(true.B)
          dut.io.expanded.expect(entry.expanded.U(32.W))
  ...
  it should "decode the quadrant-1 funct3=001 alias according to XLEN" in {
    val raw = 0x3575
  ...
  it should "keep RV64 C.ADDIW with rd=x0 reserved" in {
    simulate(new RvcDecompressor(64)) { dut =>
