...
class SupervisorExternalInterruptCoreSpec extends AnyFlatSpec with Matchers with ChiselSim {
  behavior of "AetherCore Supervisor external interrupts"

  private val base = BigInt("80000000", 16)
  private val handler = base + 0x100
  private val supervisorEntry = base + 0x38
  ...
  it should "delegate SEIP into S-mode and return with SRET" in {
    val program = Map(
      // stvec = handler
      ...
      // Supervisor loop. The interrupt must preserve this architectural PC.
      ...
      while ((!resumed || !sawCause || !sawEpc) && cycles < 320) {
        ...
        if (dut.io.commit.valid.peek().litToBoolean) {
          ...
          if (dut.io.commit.rdWrite.peek().litToBoolean) {
            ...
            if (rd == 7 && value == expectedEpc) sawEpc = true
          }

          if (sawSret && dut.io.commit.pc.peek().litValue == expectedEpc) {
            resumed = true
          }
        }
