...
/** Focused combinational proof for the policy actually used by the backend. */
trait V2P8PreHeadSafetyGateChecks { this: AnyFlatSpec with Matchers with ChiselSim =>
  ...
  it should "suppress all PTW traffic while the accepted Load is still speculative" in {
    simulate(new TinyPreHeadSafetyGate) { dut =>
      initialize(dut)
  ...
  it should "permit only replay-safe Reads before head" in {
    simulate(new TinyPreHeadSafetyGate) { dut =>
      initialize(dut)
      dut.io.speculative.poke(true.B)
      dut.io.memoryValid.poke(true.B)
  ...
  it should "not grant a speculative request merely from safe attributes when no request is live" in {
    simulate(new TinyPreHeadSafetyGate) { dut =>
      initialize(dut)
      dut.io.speculative.poke(true.B)
      dut.io.attributes.idempotent.poke(true.B)
