...
class RV64MDecoderSpec extends AnyFlatSpec with Matchers with ChiselSim {
  ...
  private case class DecodeCase(name: String, opcode: Int, funct3: Int, wordOp: Boolean)

  private val cases = Seq(
    DecodeCase("MUL",    0x33, 0x0, false),
    DecodeCase("MULH",   0x33, 0x1, false),
    DecodeCase("MULHSU", 0x33, 0x2, false),
    DecodeCase("MULHU",  0x33, 0x3, false),
    DecodeCase("DIV",    0x33, 0x4, false),
    DecodeCase("DIVU",   0x33, 0x5, false),
    DecodeCase("REM",    0x33, 0x6, false),
    DecodeCase("REMU",   0x33, 0x7, false),
    DecodeCase("MULW",   0x3b, 0x0, true),
    DecodeCase("DIVW",   0x3b, 0x4, true),
    DecodeCase("DIVUW",  0x3b, 0x5, true),
    DecodeCase("REMW",   0x3b, 0x6, true),
    DecodeCase("REMUW",  0x3b, 0x7, true)
  )
  ...
  it should "accept all RV64M register encodings with the correct width class" in {
