...
/** End-to-end proof that the experimental production backend really launches
  * conservative younger Loads, while PMA still controls externalization.
  */
trait V2P8PreHeadBackendChecks { this: AnyFlatSpec with Matchers with ChiselSim =>
  ...
  private def initialize(dut: TinyPreHeadMemoryBackend): Unit = {
    ...
    // Hold the physical request at the public seam so its timing is observable.
  ...
  private def dispatchDivThenLoad(dut: TinyPreHeadMemoryBackend, basePc: BigInt): Unit = {
    // Iterative DIVU keeps age0 live long enough to observe the age1 Load.
    dispatch(dut) {
      pokeDispatch(
        dut,
        pc = basePc,
  ...
  it should "externalize a replay-safe younger Load before the older DIV retires" in {
    simulate(new TinyPreHeadMemoryBackend(config, PageTableGeometry.Sv32)) { dut =>
      ...
      while (!sawRequest && cycles < 16) {
        // The older DIV must still own the retirement boundary when the younger
        // replay-safe Load reaches the public physical-memory seam.
